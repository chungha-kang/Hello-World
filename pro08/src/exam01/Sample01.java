...
public class Sample01 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < s1.length(); i++) {
			char c1 = s1.charAt(i);
			System.out.println("\"문자열\" 문자열에서 추출 : " + c1);
		}
		...
		System.out.println("\"문자열\" \"문자\" 가 포함되어 있으면 그 위치가 어디냐? : " + index);
		...
		// 전화번호를 입력 받아서 출력할 때 전화번호의 뒷자리 4자리는 **** 로 마스킹 하시오.
		
		
		s1 = "		앞/뒤로 공백이 포함된 문자열		";
		s1 = s1.trim(); 		// 앞/뒤의 공백을 제거 시켜줌	
		System.out.println(s1);
		...
		s1 = "010-1234-5678";
		boolean isPhone = s1.matches("\\d(3)-\\d(4)-\\d(4)");	// 정규표현식 -> 이메일, 패스워드, 아이디 등
