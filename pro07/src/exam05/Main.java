...
public class Main {
	...
	public static void main(String[] args) {
		/*
		 * 다형성
		 * 		- 객체지향 프로그래밍의 3대 특징 중 하나 (캡슐화, 상속, 다형성)
		 * 		- 하나의 객체가 여러 형태를 가진다는 의미 ( 사람(객체) -> 군인, 학생, 선생, 회사원)
		...
		// 자식 클래스(Teacher, Student)가 부모 클래스(Person)로 업 캐스팅 됨. 따로 업 캐스팅 연산자를 사용 할 필요 없음
		
		Person[] p = new Person[3];
		p[0] = new Teacher("김철원", 28);
		p[1] = new Student("홍길동", 16);
		p[2] = new Student("박장석", 16);
		// Person 배열에 Teacher, Student 를 담아주면서 업 캐스팅 함.
